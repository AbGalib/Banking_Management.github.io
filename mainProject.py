...
1. Galib Abdullah Noor (201-15-13632)
2. Most. Tahira Tabassum  (201-15-13679)
3. Rahat Uddin (201-15-13629)
4. Aklema Akther Shorna (201-15-13982)
    ...
    for line in customer_database:
        if identity == line.replace("\n", ""):
            if choice == 1:
                ...
                output_message += "NID : " + customer_database.__next__().replace("\n", "") + "\n"
            else:
...
def transaction(identity, amount, choice):  # choice 1 for deposit; choice 2 for withdraw
    ...
    for line in customer_database:
        if identity == line.replace("\n", ""):
            ...
            if choice == 2 and balance - amount < 500:  # Minimum balance 10000
                return -1
            else:
                if choice == 1:
                    balance += amount
...
class welcomeScreen:
    def __init__(self, window=None):
        ...
        window.title("Welcome to DIU Student Bank")
                            ...
                            if gender == "Male" or gender == "Female":
                                if nationality.__len__() != 0:
                                    if NID.__len__() != 0:
                                        ...
                                        Error(Toplevel(self.master))
                                        Error.setMessage(self, message_shown="NID NO!")
                                        return
                                else:
                                    Error(Toplevel(self.master))
...
class depositMoney:
    ...
    def submit(self, amount):
        ...
        if output == -1:
            ...
            output = "Amount of TK " + str(amount) + " deposited successfully.\nUpdated balance : " + str(output)
...
class withdrawMoney:
    ...
    def submit(self, amount):
        ...
        if output == -1:
            ...
            output = "Amount of TK " + str(amount) + " withdrawn successfully.\nUpdated balance : " + str(output)
            customerMenu.printMessage_outside(output)
            self.master.withdraw()
